...
--
-- Add transcription_uuid column to form_ai_summary table
-- This ensures that a transcription is only used once
--
SET @col_exists = 0;
SELECT COUNT(*) INTO @col_exists
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = DATABASE()
  AND TABLE_NAME = 'form_ai_summary'
  AND COLUMN_NAME = 'transcription_uuid';

SET @sql = IF(@col_exists = 0,
    'ALTER TABLE `form_ai_summary`
     ADD COLUMN `transcription_uuid` VARCHAR(36) DEFAULT NULL
     COMMENT ''UUID of the voice transcription to prevent reuse''
     AFTER `encounter_uuid`',
    'SELECT ''Column transcription_uuid already exists''');

PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Add index on transcription_uuid if it doesn''t exist
SET @idx_exists = 0;
SELECT COUNT(*) INTO @idx_exists
FROM INFORMATION_SCHEMA.STATISTICS
WHERE TABLE_SCHEMA = DATABASE()
  AND TABLE_NAME = 'form_ai_summary'
  AND INDEX_NAME = 'transcription_uuid_index';

SET @sql = IF(@idx_exists = 0,
    'ALTER TABLE `form_ai_summary`
     ADD KEY `transcription_uuid_index` (`transcription_uuid`)',
    'SELECT ''Index transcription_uuid_index already exists''');
